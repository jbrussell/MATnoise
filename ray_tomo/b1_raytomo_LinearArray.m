...
% Np = 5; % # of subplot columns
Mp = 1; % # of subplot rows
Np = 3; % # of subplot columns
...
ixsp = 0;
for ista = 1:length(sta.name)
    xspfiles = dir([Xsp_path,'*',sta.name{ista},'_xsp.mat']);

    for ii = 1:length(xspfiles)

        % Before loading file, extract station names from string and check whether we want to process them
        toks = strsplit(xspfiles(ii).name,'_');
        sta1 = toks{1};
        sta2 = toks{2};

        % Skip over data not contained in station list
        if ~any(strcmp(sta1,sta.name)) || ~any(strcmp(sta2,sta.name))
            continue
        end
        % Skip over data that's crossing between arrays (this is a SESAME-specific naming convention)
        if ~strcmp(sta1(4),sta2(4))
            continue
        end

        temp = load([Xsp_path,xspfiles(ii).name]);
