...
function [alpha, dlnbeta_dx, dlnbeta_dy, beta_tau, azi_maxamp, alpha_err, dlnbeta_dx_err, dlnbeta_dy_err, beta_tau_err, azi_maxamp_err]=run_fit_alpha_beta_bootstrap_gammacorr(azi,gamma,amp,amp_err,Nbs)
    ...
    for ibs = 1:Nbs
        ...
        [~, alpha_bs(ibs), dlnbeta_dx_bs(ibs), dlnbeta_dy_bs(ibs)]=fit_alpha_beta_gammacorr(azi(:),gamma(:),amp_bs(:),amp_err(:));
    end
