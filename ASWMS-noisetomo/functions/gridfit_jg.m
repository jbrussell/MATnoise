function [zgrid,xgrid,ygrid] = gridfit_jg(x,y,z,xnodes,ynodes,varargin)
...
%
% Modified by Ge Jin to releaze del^4 option
% jinwar@gmail.com
% Date: 03/2012
...
if (params.tilesize < max(nx,ny))
  ...
  switch params.regularizer
    ...
    case {'diffusion' 'laplacian'}
     ...
     case {'del4'}
      % del^4 regulation, only accurate for even spacing!!
	  [i,j] = meshgrid(1:nx,2:(ny-1));
      ind = j(:) + ny*(i(:)-1);
      dy1 = dy(j(:)-1)/params.yscale;
      dy2 = dy(j(:))/params.yscale;
      
      Areg2 = sparse(repmat(ind,1,3),[ind-1,ind,ind+1], ...
        xyRelativeStiffness(2)*[-2./(dy1.*(dy1+dy2)), ...
        2./(dy1.*dy2), -2./(dy2.*(dy1+dy2))],ngrid,ngrid);
      ...
      Areg2 = Areg2 + sparse(repmat(ind,1,3),[ind-ny,ind,ind+ny], ...
        xyRelativeStiffness(1)*[-2./(dx1.*(dx1+dx2)), ...
        2./(dx1.*dx2), -2./(dx2.*(dx1+dx2))],ngrid,ngrid);

      [i,j] = meshgrid(1:nx,3:(ny-3));
      ind = j(:) + ny*(i(:)-1);
      dy_2 = dy(j(:)-2)/params.yscale;
      dy_1 = dy(j(:)-1)/params.yscale;
      dy0 = dy(j(:))/params.yscale;
      dy1 = dy(j(:)+1)/params.yscale;
      dy2 = dy(j(:)+2)/params.yscale;
      
      Areg = sparse(repmat(ind,1,5),[ind-2,ind-1,ind,ind+1,ind+2], ...
        xyRelativeStiffness(2)*[2./(dy_2.*(dy_2+dy_1)), ...
		-8./(dy_1.*(dy_1+dy0)), 6./(dy0.*dy1), ...
        -8./(dy1.*(dy1+dy0)), 2./(dy2.*(dy1+dy2))],ngrid,ngrid);
      
      [i,j] = meshgrid(3:(nx-3),1:ny);
      ind = j(:) + ny*(i(:)-1);
      dx_2 = dx(i(:)-2)/params.xscale;
      dx_1 = dx(i(:)-1)/params.xscale;
      dx0 = dx(i(:))/params.xscale;
      dx1 = dx(i(:)+1)/params.xscale;
      dx2 = dx(i(:)+2)/params.xscale;
      
      Areg = Areg + sparse(repmat(ind,1,5),[ind-2*ny,ind-ny,ind,ind+ny,ind+2*ny], ...
        xyRelativeStiffness(1)*[2./(dx_2.*(dx_2+dx_1)), ...
		-8./(dx_1.*(dx_1+dx0)), 6./(dx0.*dx1), ...
        -8./(dx1.*(dx1+dx0)), 2./(dx2.*(dx1+dx2))],ngrid,ngrid);
