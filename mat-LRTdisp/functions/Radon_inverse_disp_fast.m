function [R,Rfft,fvec]=Radon_inverse_disp_fast(t,delta,M,p,weights,ref_dist,f_min,f_max,line_model,inversion_model,hyperparameters)
    % J. Russell
    ...
    %
      ...
      % JBR
      dF=1/(t(1)-t(2));
      f_vec = ((  [1:floor((iF+1)/2)]  -1)/iF)*dF*-1;
      f_ind = find(f_vec>=f_min*0.9 & f_vec<=f_max*1.1);
      iF_ind = length(f_ind)*2;
      
      % Preallocate space in memory.
      R=zeros(ip, it); %#ok<NASGU>
      Rfft=zeros(ip, iF_ind); %iF);
      A=zeros(iDelta,ip);
      Tshift=A;
      ...
      ii = 0;
      for i=f_ind(:)'%1:floor((iF+1)/2)
          ii = ii + 1;
          
          % Make time-shift matrix, A.
          f=f_vec(i)*-1; %((i-1)/iF)*dF;
          ...
          if(strcmpi(inversion_model, 'Cauchy')||strcmpi(inversion_model, 'L1'))
              ...
              if(strcmpi(inversion_model, 'Cauchy'))
                  COST_prev=norm(Mfft(:,i)-A*Rfft(:,ii), 2)+lambda*sum(log( abs(Rfft(:,ii) ).^2 + b ));
              elseif(strcmpi(inversion_model, 'L1'))
                  COST_prev=norm(Mfft(:,i)-A*Rfft(:,ii), 1)+lambda*norm(abs(Rfft(:,ii))+b, 2);
              end;
              iter=1;
              ...
              while( (dCOST > 0.001)&&(iter<20) )
                  ...
                  if(strcmpi(inversion_model, 'Cauchy'))
                      Q=spdiags( 1./( abs(Rfft(:,ii) ).^2 + b), 0, ip, ip  );
                  elseif(strcmpi(inversion_model, 'L1'))
                      Q=spdiags( 1./( abs(Rfft(:,ii) ) + b), 0, ip, ip  );
                  end;
                  Rfft(:,i)=( lambda*Q+AtA )\AtM;
                  
                  % Determine change to cost function.
                  if(strcmpi(inversion_model, 'Cauchy'))
                      COST_cur=norm(Mfft(:,i)-A*Rfft(:,ii), 2)+lambda*sum(log( abs(Rfft(:,ii) ).^2 + b )-log(b));
                  elseif(strcmpi(inversion_model, 'L1'))
                      COST_cur=norm(Mfft(:,i)-A*Rfft(:,ii), 1)+lambda*norm(abs(Rfft(:,ii))+b, 2);
                  end;
          ...
          if(i ~= 1)
              Rfft(:,iF_ind-ii+2)=conj(Rfft(:,ii));
          end
      end;
      
      R=ifft(Rfft,iF_ind,2, 'symmetric');
    %   R=R(:,1:it);
    return;
