...
%% Fit alpha at each frequency of interest
figure(18); clf;
rmax = 400;
for ip = 1:length(mat)
    omega = mat(ip).omega;
    ...
    c = mat(ip).c;
    alph_start = 1e-3;
    mat(ip).alph_start = alph_start;
    
    Igood = r<=rmax;
    r = r(Igood);
    xsp_real = xsp_real(Igood);
    for ii = 1:2
        %%% - Invert for the bessel function 2x - %%%
        options = optimoptions(@lsqnonlin,'TolFun',1e-12,'MaxIter',1500,'MaxFunEvals',1500);
%         [tw,~,res,~,~,~,J] = lsqnonlin(@(x) besselerr_dist(x,[xsp_real],omega,r,is_normbessel),[tw],[tw]*0.8,[tw]*1.2,options);
%         c_start = r./tw;
        [alph,~,res,~,~,~,J] = lsqnonlin(@(x) besselerr_alpha(x,c,[xsp_real],omega,r),[alph_start],[alph_start]*1e-1,[alph_start]*1e1,options);
        alph_start = alph;
    end
    % ESTIMATE ERROR BARS ON MODEL! :: JBR - 2/2020
    % Calculate data variance from residual following Menke QMDA book eq. (4.31)
    % s_d^2 = E / (N-M)
    sigma_d2 = res'*res / (length(res)-length(alph));
    Cov_m = inv(J'*J)*sigma_d2;
    sigma_m_alph = diag(Cov_m).^(1/2);
    
%     mat(ip).c = r./tw;
    mat(ip).alph = alph;
    mat(ip).alph_std = sigma_m_alph;
    
    figure(18);
    subplot(2,1,1);
    plot(mat(ip).period,mat(ip).alph_start,'ok'); hold on;
    errorbar(mat(ip).period,mat(ip).alph,mat(ip).alph_std,'.r');
    xlabel('Period (s)');
    ylabel('Alpha (km^{-1})');
end
