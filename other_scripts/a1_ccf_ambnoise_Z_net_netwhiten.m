...
% {datadirectory}/{network}/{station}/{station}.{yyyy}.{jday}.{hh}.{mm}.{SS}.{COMP}.sac
...
%
% JBR, 11/2025: This version whitens each station's spectrum using the same
% network-average spectrum to ensure each station is treated equally. This should
% preserve relative amplitudes among all station pairs.
% Must first run a00a and a00b to calculate network average spectrum.
%
...
% Load network average spectrum
netspec = load(['./psd/window3hr/fullStack/psdZZ/psd_network_average.mat']);
...
ccf_winlength_path = [ccf_path,'window',num2str(winlength),'hr_netwhiten/'];
...
fig_winlength_path = [figpath,'window',num2str(winlength),'hr_netwhiten/'];
if ~exist(figpath)
    mkdir(figpath);
end
...
seis_winlength_path = [seis_path,'window',num2str(winlength),'hr_netwhiten/'];
if ~exist(seis_path)
    mkdir(seis_path);
end
...
for ista1=1:nsta
    ...
    for ista2=1:nsta
        ...
        for ifil = 1:length(list1)
            ...
            data2cZ= dir([datadir,'/',net2,'/',stacode2,'/',year,'/',stacode2,'.',hdayid,'.*',strSACcomp,'.sac']);
            ...
            try
                ...
                disp(['Something wrong sac file... skipping ',data2cZ])
                continue
            end
...
%                 coh_trace = coh_trace ./ abs(fftS1Z) ./ abs(fftS2Z);
                coh_trace = coh_trace ./ netspec.psd_avg ./ netspec.psd_avg; % network whitening
