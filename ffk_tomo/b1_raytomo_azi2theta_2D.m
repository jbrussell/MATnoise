% Script to do the finite-frequency tomography based on the ambient noise measurement. 
% Solves for azimuthal anisotropy and isotropic velocity on a 2D grid.
%
% Finite frequency kernels are calculated according to equation (11) or (12)
% from Lin & Ritzwoller (2010). To use the analytical kernel, set tphase_in = [].
% Currently, the true empirical kernel has not been tested.
%
...
% Kernel parameters
kernel_path = parameters.kernel_path; % path to kernel output
kmode = parameters.kmode; % <0 : bandwidth; =0 instantaneous; >0 Fresnel zones
nfreqs = parameters.nfreqs; % number of frequencies to include in bandwidth average
bw_frac = parameters.bw_frac; % fraction of frequency to include in bandwidth
tphase_in = parameters.tphase_in; % Path to SEM traveltime surfaces
...
%% Damping of isotropic structure toward vref
Fdamp = sparse(Nx*Ny,Nx*Ny+Nxyazi);
Fdamp(1:Nx*Ny,1:Nx*Ny) = eye(Nx*Ny,Nx*Ny);
...
for ip=1:length(Tperiods)
    ...
    clear rays stas dt fiterr mat phaseg err raydense dist azi mat_azi phv phv_std dt_std
    raynum = 0;
...
%     mat_iso=ray_kernel_build(rays,xnode,ynode);
    ...
    % First pass, use analytical kernel
    mat_iso=ffrq_2dkernel_build(kmode, rays, nfreqs,bw_frac,freq,phv, ynode, xnode, stas,[]);
    raymat = ray_kernel_build(rays,xnode,ynode); % kernel should be great circle approx.
     
%     mat_iso=ffrq_2dkernel_build(kmode, rays, nfreqs,bw_frac,freq,vref, ynode, xnode, stas,tphase_in);
%     ynodek = min(ynode):0.05:max(ynode);
%     xnodek = min(xnode):0.05:max(xnode); 
%     dtds = ffrq_2dkernel_build(1, rays, [],[],freq,vref, ynodek, xnodek, stas,tphase_in);
%     raymat = raybend_kernel_build(rays, xnode, ynode,dtds*-1,xnodek,ynodek,freq);
    
    toc
    
    % update phv with rays from kernel
    r_ray = sum(raymat,2);
    phv = r_ray./dt;
    
    % Build azimuthal part of data kernel
    % mat_azi = r_ray .* [cosd(2*azi'), sind(2*azi'), cosd(4*azi'), sind(4*azi') ];
    [mat_azi, mat_azi_hits] = ray_kernel_build_azi(rays,xnode_azi,ynode_azi);
    
    % JBR - Combine isotropic and anisotropic
    mat = [mat_iso, mat_azi];
    
    % Set up data vector so constraint equations apply to full model
    s0 = ones(size(mat,2),1);
    s0(1:Nx*Ny) = 1./vref;
    s0([1:Nxyazi]+Nx*Ny) = 0;
%     dtpre = dist' .* mean(s0(1:Nx*Ny));
%     dtpre = gc_raytrace( s0(1:Nx*Ny), rays, xnode, ynode );
    dtpre = raymat*s0(1:Nx*Ny);
    ddt = (dt - dtpre) + mat*s0;
...
%         W(i,i)=1./dt_std(i);
    ...
    grid_wl = 1;
    if is_wlsmooth == 1 % weight smoothing by wavelength 0:no, 1:yes
        grid_wl = vref/freq/deg2km(gridsize); % grid cells per wavelength
    end
    
    % calculate the smoothing weight
    NR=norm(F,1);
    NA=norm(W*mat,1);
    smweight = smweight0*NA/NR * grid_wl;
    
    % Rescale the damp kernel
    NR=norm(Fdamp,1);
    NA=norm(W*mat,1);
    dampweight = damp0*NA/NR;
    ...
    NR=norm(F_azic,1);
    NA=norm(W*mat,1);
    smweight_azi = smweight0_azi*NA/NR * grid_wl;
    ...
    disp('start inverse');
    A=[W*mat; smweight*F; dampweight*Fdamp; damp_azi*F_azi_damp; smweight_azi*F_azic; smweight_azi*F_azis; flweight_azi*J_azic; flweight_azi*J_azis];
    rhs=[W*ddt; zeros(size(F,1),1); dampweight*ones(size(Fdamp,1),1).*s0(1:Nx*Ny); zeros(size(F_azi_damp,1),1); zeros(size(F_azic,1),1); zeros(size(F_azis,1),1); zeros(size(J_azic,1),1); zeros(size(J_azis,1),1)];
%     A=[W*mat; smweight*F; dampweight*Fdamp; aziweight*F_azi_damp*damp0_azi_fac; smweight_azi*F_azic; smweight_azi*F_azis; flweight_azi*J_azic; flweight_azi*J_azis];
%     rhs=[W*ddt; zeros(size(F,1),1); dampweight*ones(size(Fdamp,1),1).*s0(1:Nx*Ny); zeros(size(F_azi_damp,1),1); zeros(size(F_azic,1),1); zeros(size(F_azis,1),1); zeros(size(J_azic,1),1); zeros(size(J_azis,1),1)];
    ...
    %%
    % Second pass, use empirical kernels
    vref = 1./mean(phaseg(1:Nx*Ny));
    
    tic
    
    % Update ray lengths using bent rays extracted from kernel
    ynodek = min(ynode):0.05:max(ynode);
    xnodek = min(xnode):0.05:max(xnode); 
    dtds = ffrq_2dkernel_build(1, rays, [],[],freq,phv, ynodek, xnodek, stas,tphase_in);
    raymat = raybend_kernel_build(rays, xnode, ynode,dtds*-1,xnodek,ynodek,freq);
    ...
    % Empirical kernels
    mat_iso=ffrq_2dkernel_build(kmode, rays, nfreqs,bw_frac,freq,phv, ynode, xnode, stas,tphase_in);
    ...
    % Update traveltime predictions using empirical kernels
    dtpre_iso = dtpre + mat_iso*(phaseg(1:Nx*Ny)-s0(1:Nx*Ny));
    dtpre = dtpre + mat*(phaseg-s0);
%     dtpre_iso = raymat*phaseg(1:Nx*Ny);
%     dtpre = [raymat,mat_azi]*phaseg;
    ...
    while niter < 2
        ...
        disp(['Bad Measurement Number: ', num2str(length(ind))]);
        for i=1:length(err)
            if abs(err(i)) > stderrfac*stderr %dterrtol*stderr %2*stderr
                W(i,i)=0;
            end
        end
        ind = find(diag(W)==0);
        ...
        % Invert
        % A=[W*mat; smweight*F; dampweight*Fdamp; aziweight*F_azi_damp*damp0_azi_fac];
        % rhs=[W*ddt; zeros(size(F,1),1); dampweight*ones(size(Fdamp,1),1).*s0(1:Nx*Ny); zeros(size(F_azi_damp,1),1)];
%         A=[W*mat; smweight*F; dampweight*Fdamp; aziweight*F_azi_damp*damp0_azi_fac; smweight_azi*F_azic; smweight_azi*F_azis; flweight_azi*J_azic; flweight_azi*J_azis];
%         rhs=[W*ddt; zeros(size(F,1),1); dampweight*ones(size(Fdamp,1),1).*s0(1:Nx*Ny); zeros(size(F_azi_damp,1),1); zeros(size(F_azic,1),1); zeros(size(F_azis,1),1); zeros(size(J_azic,1),1); zeros(size(J_azis,1),1)];
        A=[W*mat; smweight*F; dampweight*Fdamp; damp_azi*F_azi_damp; smweight_azi*F_azic; smweight_azi*F_azis; flweight_azi*J_azic; flweight_azi*J_azis];
        rhs=[W*ddt; zeros(size(F,1),1); dampweight*ones(size(Fdamp,1),1).*s0(1:Nx*Ny); zeros(size(F_azi_damp,1),1); zeros(size(F_azic,1),1); zeros(size(F_azis,1),1); zeros(size(J_azic,1),1); zeros(size(J_azis,1),1)];
        ...
        % update predicted phase velocities
        dtpre_iso = dtpre_iso + mat_iso*(phaseg(1:Nx*Ny)-s0(1:Nx*Ny));
        dtpre = dtpre + mat*(phaseg-s0);
%         dtpre_iso = raymat*phaseg(1:Nx*Ny);
%         dtpre = [raymat,mat_azi]*phaseg;
    ...
    % Update traveltime prediction
    dtpre_gc = gc_raytrace( phaseg(1:Nx*Ny), rays, xnode, ynode );
    dtpre_ray = raymat*phaseg(1:Nx*Ny);
    dtpre_iso = dtpre_iso + mat_iso*(phaseg(1:Nx*Ny)-s0(1:Nx*Ny));
    dtpre = dtpre + mat*(phaseg-s0);
    
    %%    
    % Isotropic phase velocity
%     phv_iso = dist'./dtpre_iso;
    phv_iso = r_ray./dtpre_iso;
%     phv_iso = r_ray./dtpre_ray;
...
%     slow_av = 1/mean(phv_iso);
    ...
    dphv = (phv - phv_iso) ./ phv_iso; % remove isotropic velocities
    ...
    raytomo(ip).res = res;
    raytomo(ip).mat = mat;
    raytomo(ip).raydense_azi = raydense_azi;
...
%     raytomo(ip).phi4 = phi4;
    ...
    raytomo(ip).R2 = R2;
    raytomo(ip).r_ray = r_ray;
    raytomo(ip).dphv = dphv;
    raytomo(ip).phv_obs = harmmean(raytomo(ip).phv(raytomo(ip).isgood));
    raytomo(ip).phv_obs_std = std(raytomo(ip).phv(raytomo(ip).isgood));
...
subplot(Mp,Np,ii)
    ax = worldmap(lalim, lolim);
    set(ax, 'Visible', 'off')
    surfacem(xi,yi,raytomo(ip).raydense_azi ./ deg2km(gridsize));
%     drawlocal
    title([num2str(Tperiods(ip))],'fontsize',15)
    cb = colorbar;
